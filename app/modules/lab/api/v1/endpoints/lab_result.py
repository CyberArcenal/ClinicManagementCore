...
from app.modules.lab.models.lab import LabStatus
...
async def create_lab_request(
    ...
    try:
        ...
        return success_response(data=lab, message="Lab request created")
...
async def list_lab_results(
...
) -> SuccessResponse[PaginatedResponse[LabResultResponse]]:
    filters = {}
    if patient_id:
    ...
    return success_response(data=paginated, message="Lab results retrieved")
...
async def get_lab_result(
    ...
    if current_user.role == "patient":
        ...
        if not doctor_profile or lab.requested_by_id != doctor_profile.id:
            raise HTTPException(status_code=403, detail="Access denied")
    return success_response(data=lab, message="Lab result retrieved")
...
async def update_lab_result(
    ...
    try:
        ...
        if not lab:
            raise HTTPException(status_code=404, detail="Lab result not found")
        return success_response(data=lab, message="Lab result updated")
...
async def start_lab_processing(
    ...
    try:
        ...
        if not lab:
            raise HTTPException(status_code=404, detail="Lab result not found")
        return success_response(data=lab, message="Lab processing started")
    except (InvalidLabStatusTransitionError, LabTechNotFoundError) as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.patch("/{lab_id}/complete")
async def complete_lab_result(
    ...
    try:
        ...
        if not lab:
            raise HTTPException(status_code=404, detail="Lab result not found")
        return success_response(data=lab, message="Lab result completed")
    except InvalidLabStatusTransitionError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.patch("/{lab_id}/cancel")
async def cancel_lab_request(
    ...
    try:
        ...
        if not lab:
            raise HTTPException(status_code=404, detail="Lab result not found")
        return success_response(data=lab, message="Lab request cancelled")
    except InvalidLabStatusTransitionError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_patient_lab_history(
    ...
    return success_response(data=history, message="Patient lab history retrieved")
...
async def get_pending_lab_requests(
    ...
    return success_response(data=pending, message="Pending lab requests retrieved")