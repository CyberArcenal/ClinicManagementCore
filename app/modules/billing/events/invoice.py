...
def register_invoice_events():
    ...
    def capture_invoice_changes(session, flush_context, instances):
        pending = session.info.setdefault('invoice_pending', {'new': [], 'updated': [], 'deleted': []})
        ...
        for obj in session.dirty:
            if isinstance(obj, Invoice):
                ...
                if changes:
                    InvoiceTransition(session).on_before_update(obj, changes)
        ...
        for obj in session.deleted:
            if isinstance(obj, Invoice):
                InvoiceTransition(session).on_before_delete(obj)
                pending['deleted'].append(obj)

    @event.listens_for(SQLSession, 'after_commit')
    def process_invoice_after_commit(session):
        pending = session.info.pop('invoice_pending', None)
        if not pending:
            return
        ts = InvoiceTransition(session)
