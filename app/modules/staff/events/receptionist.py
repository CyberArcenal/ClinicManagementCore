...
def register_receptionist_events():
    # Same pattern (copy nurse pattern, change class names)
    @event.listens_for(SQLSession, 'before_flush')
    def capture_receptionist_changes(session, flush_context, instances):
        pending = session.info.setdefault('receptionist_pending', {'new': [], 'updated': [], 'deleted': []})
        for obj in session.new:
            if isinstance(obj, ReceptionistProfile):
        ...
        for obj in session.dirty:
            if isinstance(obj, ReceptionistProfile):
                ...
                if changes:
                    ReceptionistProfileTransition(session).on_before_update(obj, changes)
                    pending['updated'].append({'instance': obj, 'changes': changes})
        for obj in session.deleted:
            if isinstance(obj, ReceptionistProfile):
                ReceptionistProfileTransition(session).on_before_delete(obj)
                pending['deleted'].append(obj)

    @event.listens_for(SQLSession, 'after_commit')
    def process_receptionist_after_commit(session):
        pending = session.info.pop('receptionist_pending', None)
        if not pending:
            return
        ts = ReceptionistProfileTransition(session)
